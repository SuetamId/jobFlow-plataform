    ...
    <div className="space-y-6">
      <Card className="p-6">
        <div className="flex items-start gap-5">
          <div className="w-16 h-16 rounded-xl bg-background-secondary flex items-center justify-center overflow-hidden flex-shrink-0 border border-border">
            {job.company.logo ? (
              <img
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-lg text-foreground-secondary mt-0.5">{job.company.name}</p>
          </div>
        </div>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          {job.hasRemote && <Badge variant="success">Remote</Badge>}
          {job.workModelLabel && (
      ...
      <Card className="p-6">
        ...
        <div
          className="mt-4 prose prose-sm max-w-none text-foreground-secondary prose-headings:text-foreground prose-a:text-secondary"
        ...
        <div className="mt-4 flex flex-wrap gap-4">
          {job.company.websiteUrl && (
            <a
              ...
              className="inline-flex items-center gap-1.5 text-sm text-foreground-secondary hover:text-secondary transition-colors"
            >
          ...
          {job.company.githubUrl && (
            <a
              href={job.company.githubUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 text-sm text-foreground-secondary hover:text-secondary transition-colors"
            >
          ...
          {job.company.twitterHandle && (
            <a
              href={`https://twitter.com/${job.company.twitterHandle}`}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 text-sm text-foreground-secondary hover:text-secondary transition-colors"
            >
...
function InfoItem({ icon, label, value }: InfoItemProps) {
  return (
    <div className="flex items-center gap-3 p-3 rounded-lg bg-background-secondary">
      ...
      <div>
        <p className="text-xs font-medium text-foreground-tertiary uppercase tracking-wide">{label}</p>
